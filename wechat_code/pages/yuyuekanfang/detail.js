...
this.setData({
id: myid,
isAuth:utils.isAuth('yuyuekanfang','审核'),
isAuthFront:  utils.isAuthFront('yuyuekanfang','审核'),
...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.fangwumingcheng,
...
const {
data
} = await detail("yuyuekanfang",id)
this.setData({
payAuth:utils.isAuthFront('yuyuekanfang','支付')
})
...
const resUpdate = await update('yuyuekanfang', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','yuyuekanfang');
...
const res = await update("yuyuekanfang", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','yuyuekanfang');
