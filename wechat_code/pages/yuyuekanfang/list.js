...
if(this.data.fangwumingcheng){
    searchForm['fangwumingcheng'] = '%' + this.data.fangwumingcheng + '%'
}
let res = {};
if(this.data.userid) {

res = await page(`yuyuekanfang`, searchForm);
} else {
res = await list(`yuyuekanfang`, searchForm);
}
...
let targetName="fangwumingcheng"
...
wx.navigateTo({
url: `/pages/yuyuekanfang/update-and-add`
})
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("yuyuekanfang",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/yuyuekanfang/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/yuyuekanfang/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("yuyuekanfang", "新增")
isAuthObj['delAuth']  = utils.isAuth("yuyuekanfang", "删除")
isAuthObj['editAuth']   =utils.isAuth("yuyuekanfang", "修改")

}else{
isAuthObj['addAuth']   = utils.isAuthFront("yuyuekanfang", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("yuyuekanfang", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("yuyuekanfang", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("yuyuekanfang",obj)
    }else{
         resList= await list("yuyuekanfang",obj)
    }
