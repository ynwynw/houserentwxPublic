...
const {
data
} = await detail("zulinhetong",id)
this.setData({
payAuth:utils.isAuthFront('zulinhetong','支付')
})
...
const resUpdate = await update('zulinhetong', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zulinhetong');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zulinhetong", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','zulinhetong');
