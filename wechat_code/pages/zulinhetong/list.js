...
res = await page(`zulinhetong`, searchForm);
} else {
res = await list(`zulinhetong`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zulinhetong/update-and-add`
})
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zulinhetong",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zulinhetong/update-and-add?id=${id}&isAuth=${this.data.isAuthStatus}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
    url: `/pages/zulinhetong/detail?isAuth=${this.data.isAuthStatus}`
})
if(this.data.isAuthStatus) {
setTimeout(function () {
    wx.setStorageSync('isAuth', true)
...
const userId=       getApp().globalData.userInfo.id
        let isAuthObj={}
if(this.data.isAuthStatus){
isAuthObj['addAuth']   = utils.isAuth("zulinhetong", "新增")
isAuthObj['delAuth']  = utils.isAuth("zulinhetong", "删除")
isAuthObj['editAuth']   =utils.isAuth("zulinhetong", "修改")

}else{
isAuthObj['addAuth']   = utils.isAuthFront("zulinhetong", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("zulinhetong", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("zulinhetong", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zulinhetong",obj)
    }else{
         resList= await list("zulinhetong",obj)
    }
